            ...
            <div className="flex border-b border-white/8">
                {([
                    ...
                    <button key={key} onClick={() => { setTab(key); setSearch('') }}
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold transition-all ${tab === key ? 'text-white border-b-2 border-amber-500' : 'text-white/30 hover:text-white/60'}`}>
                        {icon} {label}
                        {count > 0 && (
                            <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${tab === key ? 'bg-amber-500/30 text-purple-200' : 'bg-white/8 text-white/40'}`}>{count}</span>
                        )}
                    </button>
                ))}
            </div>
            ...
            <div className="px-3 pt-3 pb-2">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-xl pl-8 pr-3 py-2 text-xs text-white focus:outline-none focus:border-amber-500/50 placeholder:text-white/20"
                    />
                </div>
            </div>
                            ...
                            return (
                                <button key={c.code} onClick={() => toggleCountry(c.code)}
                                    className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-medium text-left transition-all ${isSelected
                                        ? 'bg-amber-500/20 border border-amber-500/40 text-purple-200'
                                    ...
                                    {isSelected && <X size={10} className="shrink-0 text-amber-400" />}
                                </button>
                            )
                        })}
                            ...
                            return (
                                <button key={`${city.code}-${city.name}-${i}`} onClick={() => toggleCity(city)}
                                    className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs text-left transition-all ${isSelected
                                        ? 'bg-amber-500/15 border border-amber-500/30 text-blue-200'
                                        : 'bg-white/3 border border-white/8 text-white/60 hover:bg-white/8 hover:text-white/90'}`}>
                                    <MapPin size={11} className={`shrink-0 ${isSelected ? 'text-amber-400' : 'text-white/20'}`} />
