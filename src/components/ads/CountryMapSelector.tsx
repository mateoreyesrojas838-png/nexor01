        ...
        <div className="space-y-3">
                                ...
                                geographies.map((geo: any) => {
                                    ...
                                    return (
                                        <Geography
                                            ...
                                            style={{
                                                default: { fill: isSel ? '#B45309' : '#1a1a3e', stroke: '#0d0d2b', strokeWidth: 0.4, outline: 'none', cursor: info ? 'pointer' : 'default' },
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl pl-8 pr-3 py-2 text-sm text-white focus:outline-none focus:border-amber-500/50 placeholder:text-white/20"
                />
                {citySearch && (
                        ...
                        {cityResults.map(c => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-3 py-2.5 text-sm hover:bg-white/5 transition-all text-left ${isSel ? 'text-amber-400' : 'text-white/70'}`}
                                >
            ...
            {selected.length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                    {selectedCountries.map(code => (
                        <span key={code} className="flex items-center gap-1 text-xs bg-amber-500/10 border border-amber-500/20 text-amber-300 px-2.5 py-1 rounded-full">
                            {CODE_TO_NAME[code] || code}
                            <button onClick={() => toggle(code)} className="hover:text-red-400 ml-0.5"><X size={9} /></button>
                        </span>
                    ))}
                    {selectedCities.map(city => (
                        <span key={city} className="flex items-center gap-1 text-xs bg-amber-500/10 border border-amber-500/20 text-amber-300 px-2.5 py-1 rounded-full">
                            <MapPin size={9} />{city}
