    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-gradient-to-r from-yellow-500/15 to-yellow-600/5 border border-yellow-500/30 rounded-2xl p-4">
        ...
        <div className="mt-3 pt-3 border-t border-yellow-500/10 flex items-center gap-2">
          ...
          <p className="text-xs text-white/40">BNB Smart Chain · USDT-BEP20</p>
        </div>
      </div>
      ...
      {step === 'connect' && (
        <div className="flex flex-col gap-3">
          <div className="bg-white/5 border border-white/8 rounded-2xl p-4 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-white/40">
              <div className="bg-white/5 rounded-xl p-3 flex flex-col items-center gap-1">
                ...
                <span className="font-medium text-white/50">Móvil</span>
                <span className="text-center">Escanea el QR con Trust Wallet, MetaMask o Binance</span>
              </div>
              <div className="bg-white/5 rounded-xl p-3 flex flex-col items-center gap-1">
                <span className="text-lg">💻</span>
                <span className="font-medium text-white/50">PC</span>
          ...
          {onCancel && (
            <button onClick={onCancel} className="w-full py-3 rounded-xl font-bold text-sm bg-white/5 text-white/40 hover:bg-white/10 transition-all">
              Cancelar
            </button>
          )}
        </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
            ...
            <div className="bg-black/20 rounded-xl p-3 flex items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-white/30 font-mono mt-0.5 hidden sm:block">{address}</p>
              </div>
              <a
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-black/20 rounded-xl p-3">
                <p className="text-xs text-white/40 mb-1">Balance USDT</p>
                {loadingBalance ? (
              ...
              <div className="bg-black/20 rounded-xl p-3">
                <p className="text-xs text-white/40 mb-1">A pagar</p>
      ...
      {step === 'processing' && (
        <div className="flex flex-col items-center gap-4 py-6">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-white/40 mt-1">No cierres esta ventana</p>
          </div>
          {txHash && (
            <div className="bg-white/5 rounded-xl p-3 w-full text-center">
              <p className="text-xs text-white/40 mb-1">Hash de transacción</p>
      ...
      {step === 'success' && (
        <div className="flex flex-col items-center gap-4 py-6">
          ...
          {txHash && (
            <div className="bg-white/5 rounded-xl p-3 w-full text-center">
              <p className="text-xs text-white/40 mb-1">Transacción</p>
              <p className="text-xs text-white font-mono truncate">{shortAddress(txHash)}</p>
              <a
                href={`https://bscscan.com/tx/${txHash}`}
                target="_blank"
      ...
      {step === 'error' && (
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            className="w-full py-3 rounded-xl font-bold text-sm bg-white/5 text-white/50 hover:bg-white/10 transition-all"
          >
            Reintentar
          </button>
          {onCancel && (
