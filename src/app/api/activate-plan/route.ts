...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      // Leer estado actual con FOR UPDATE
      ...
      if (newRank < currentRank || newRank === 0) throw new Error('PLAN_NOT_UPGRADE')
      ...
      if (!approvedRequest) throw new Error('NO_APPROVED_PAYMENT')
    ...
    if (err.message === 'NO_APPROVED_PAYMENT') {
      return NextResponse.json({ error: 'No tienes un pago aprobado para este plan. Espera la aprobación del administrador.' }, { status: 403 })
    }
...
export async function GET() {
  try {
    const user = await getAuthUser()
    if (!user) return NextResponse.json({ error: 'No autorizado' }, { status: 401 })
    return NextResponse.json({ currentPlan: (user as any).plan ?? 'NONE' })
  } catch (err) {
