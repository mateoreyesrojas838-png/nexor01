...
import {
  ...
  Copy, Check, ExternalLink, Upload, Image as ImageIcon, X
} from 'lucide-react'
...
export default function FormBuilder() {
  ...
  const [meta, setMeta] = useState({ title: '', description: '', themeColor: '#F59E0B', coverUrl: '', notifyEmail: false, thankYouMsg: '', status: 'DRAFT' })
  ...
  const fetchForm = useCallback(async () => {
    try {
      ...
      setMeta({
        ...
        coverUrl: d.form.coverUrl || '', notifyEmail: !!d.form.notifyEmail, thankYouMsg: d.form.thankYouMsg || '', status: d.form.status,
      })
  ...
  async function saveForm(patch?: any) {
    ...
    try {
      ...
      if (patch?.status) setMeta(m => ({ ...m, status: patch.status }))
  ...
  async function uploadCover(file: File) {
    ...
    try {
      ...
      const d = await r.json()
      if (!r.ok) { setError(d.error || 'Error'); return }
      setMeta(m => ({ ...m, coverUrl: d.url })); flash('Portada subida — guardá')
    ...
    <div className="max-w-3xl">
      ...
      <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-5 mb-5 space-y-3">
        ...
        <div className="flex gap-3">
          <div className="w-32 h-20 rounded-xl overflow-hidden bg-white/5 border border-white/10 shrink-0 flex items-center justify-center">
          ...
          <div className="flex flex-col justify-center gap-2">
            <button onClick={() => coverRef.current?.click()} disabled={uploadingCover} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white/70 hover:bg-white/10 w-fit">
              {uploadingCover ? <Loader2 size={13} className="animate-spin" /> : <Upload size={13} />} Portada
            </button>
            ...
            <label className="flex items-center gap-2 text-xs text-white/60">
              Color: <input type="color" value={meta.themeColor} onChange={e => setMeta(m => ({ ...m, themeColor: e.target.value }))} className="w-8 h-7 rounded cursor-pointer bg-transparent" />
            </label>
          </div>
        </div>
        <input value={meta.title} onChange={e => setMeta(m => ({ ...m, title: e.target.value }))} placeholder="Título del formulario" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-amber-500/50" />
        <textarea value={meta.description} onChange={e => setMeta(m => ({ ...m, description: e.target.value }))} placeholder="Descripción (opcional)" rows={2} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-amber-500/50 resize-none" />
        <input value={meta.thankYouMsg} onChange={e => setMeta(m => ({ ...m, thankYouMsg: e.target.value }))} placeholder="Mensaje de gracias al enviar (opcional)" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-amber-500/50" />
        <label className="flex items-center gap-2 text-xs text-white/60 cursor-pointer">
          <input type="checkbox" checked={meta.notifyEmail} onChange={e => setMeta(m => ({ ...m, notifyEmail: e.target.checked }))} className="accent-amber-500" />
