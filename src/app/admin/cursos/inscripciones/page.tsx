...
import { ArrowLeft, Loader2, CheckCircle2, XCircle, ExternalLink, GraduationCap } from 'lucide-react'
...
export default function AdminEnrollmentsPage() {
  ...
  return (
    <div>
      ...
      <h1 className="text-xl font-black text-white flex items-center gap-2 mb-1"><GraduationCap size={20} className="text-amber-400" /> Inscripciones pendientes</h1>
      <p className="text-xs text-white/30 mb-6">Aprobá los pagos manuales. Los pagos cripto se confirman solos por el cron.</p>

      {rows.length === 0 ? (
        <div className="text-center py-16 text-white/30 text-sm">No hay inscripciones pendientes. ✓</div>
      ) : (
        <div className="space-y-3">
          {rows.map(r => (
            <div key={r.id} className="rounded-2xl border border-white/8 bg-white/[0.03] p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                <p className="font-bold text-white text-sm truncate">{r.course.title}</p>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <span className="text-[11px] text-amber-400">${r.course.price.toFixed(2)}</span>
