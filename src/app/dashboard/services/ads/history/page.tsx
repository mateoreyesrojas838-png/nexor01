...
const PLATFORM_MAP: Record<string, { letter: string; color: string }> = {
    META: { letter: 'f', color: 'text-amber-400' },
}
        ...
        <div className="px-4 md:px-6 pt-6 max-w-screen-2xl mx-auto pb-24 text-white">
            ...
            <div className="flex items-center gap-3 mb-7">
                ...
                <div className="flex items-center gap-2">
                    <Link href="/dashboard/services/ads/wizard"
                        className="hidden sm:flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-xl bg-amber-600 text-white hover:bg-amber-500 transition-all">
                        <Plus size={13} /> Nueva
                    </Link>
            ...
            {totalPublished > 0 && (
                <div className="mb-2">
                    ...
                    {[
                        { icon: <Eye size={14} />, label: 'Impresiones', value: fmt(totalImpressions), color: 'text-amber-400' },
                        { icon: <MousePointerClick size={14} />, label: 'Clics', value: fmt(totalClicks), color: 'text-amber-400' },
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-white/[0.015] border border-dashed border-white/8 rounded-3xl">
                    ...
                    <Link href="/dashboard/services/ads/wizard"
                        className="inline-flex items-center gap-2 px-5 py-2.5 bg-amber-600 text-white text-sm font-bold rounded-xl hover:bg-amber-500 transition-all">
                                        ...
                                        {loadingMetrics && !campaignMetrics ? (
                                            ...
                                            <div className="mt-3">
                                                <div className="grid grid-cols-4 gap-2">
                                                    {[
                                                        { label: 'Impresiones', value: fmt(campaignMetrics!.impressions), color: 'text-amber-400' },
                                                        { label: 'Clics', value: fmt(campaignMetrics!.clicks), color: 'text-amber-400' },
                                ...
                                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/5">
                                    {campaign.status === 'READY' && (
                                        <Link href={`/dashboard/services/ads/preview/${campaign.id}`}
                                            className="flex-1 sm:flex-none text-center text-xs font-bold px-4 py-2 rounded-xl bg-amber-600 text-white hover:bg-amber-500 transition-all">
                                            Publicar →
                                        </Link>
                                    )}
                                    ...
                                    {campaign.status === 'PUBLISHED' && campaign.providerCampaignId && (
                                        <a href="https://business.facebook.com/adsmanager" target="_blank" rel="noopener noreferrer"
                                            className="flex items-center gap-1.5 text-xs font-bold px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 hover:bg-amber-500/20 transition-all">
