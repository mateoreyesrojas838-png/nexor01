...
export default function FormBuilder() {
  ...
  const fetchForm = useCallback(async () => {
    try {
      const r = await fetch(`/api/my-forms/${formId}`)
  ...
  async function saveForm(patch?: any) {
    ...
    try {
      const r = await fetch(`/api/my-forms/${formId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patch ?? meta) })
      const d = await r.json()
  ...
  async function uploadCover(file: File) {
    ...
    try {
      ...
      const d = await r.json()
      if (!r.ok) { setError(d.error || 'Error'); return }
      setMeta((m: any) => ({ ...m, coverUrl: d.url }))
      await saveForm({ coverUrl: d.url }) // persistir ya — no depender del botón Guardar
  ...
  async function uploadVideo(file: File) {
    ...
    try {
      ...
      const d = await r.json()
      if (!r.ok) { setError(d.error || 'Error'); return }
      setMeta((m: any) => ({ ...m, headerVideoUrl: d.url }))
      await saveForm({ headerVideoUrl: d.url }) // persistir ya — no depender del botón Guardar
  ...
  async function addField() {
    const r = await fetch(`/api/my-forms/${formId}/fields`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: addType }) })
    const d = await r.json()
    if (r.ok) setFields(f => [...f, d.field])
  }
  async function patchField(id: string, data: any) {
    setFields(f => f.map(x => x.id === id ? { ...x, ...data } : x))
    await fetch(`/api/my-forms/fields/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
  }
  async function deleteField(id: string) {
    if (!confirm('¿Eliminar este campo?')) return
    setFields(f => f.filter(x => x.id !== id))
    await fetch(`/api/my-forms/fields/${id}`, { method: 'DELETE' })
  }
  ...
  async function patchFieldOrder(id: string, order: number) {
    await fetch(`/api/my-forms/fields/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ order }) })
  }
